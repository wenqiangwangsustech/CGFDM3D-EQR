...
for mpiZ in range( grid.PZ ):
	for mpiY in range( grid.PY ):
		...
		file2  = open( "%s_X_mpi_%d_%d_%d.bin" % ( fileName2 , mpiX, mpiY, mpiZ ), "rb" )
		ny = grid.ny[mpiY]
		nz = grid.nz[mpiZ]
		...
		data_2 = np.fromfile( file2 , dtype='float32', count = ny * nz )
...
plt.pcolormesh( dataY, dataZ, np.abs( data - data2 ), vmax = vm, vmin = 0.0, cmap = "seismic" )
#plt.imshow( data, vmax = vm, vmin = -vm, cmap = "jet", origin= "lower" )
#plt.pcolormesh( dataY, dataZ, data, cmap = "jet" )
