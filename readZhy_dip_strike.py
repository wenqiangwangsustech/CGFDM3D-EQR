...
zy_mom		   = np.zeros( [ lengthPoint * depthPoint ] )
zy_sample_rate = np.zeros( [ lengthPoint * depthPoint ] )
zy_slip_rate_s = np.zeros( [lengthPoint * depthPoint, NT] )
zy_slip_rate_d = np.zeros( [lengthPoint * depthPoint, NT] )
...
for j in range( depthPoint ):
	for i in range( lengthPoint ):
		...
		zy_mom		   [index] = zy_data[pos,9]
		zy_sample_rate [index] = zy_data[pos,10]
		zy_slip_rate_s [index] = zy_data[pos,11:]

		pos =  j + i * depthPoint + lengthPoint * depthPoint
		zy_slip_rate_d [index] = zy_data[pos,11:]
'''
'''

zy_slip_rate = np.sqrt( zy_slip_rate_s ** 2 + zy_slip_rate_d ** 2 )



for i in range( depthPoint * lengthPoint ): 
...
src_npts = depthPoint * lengthPoint
...
for j in range( src_npts ):
	for i in range( NT ):
		rake[j] = math.atan2( zy_slip_rate_d[j, i], zy_slip_rate_s[j, i] ) / np.pi * 180
...
sourceFileName = params["sourceDir"] + "/zhangyong.bin"
...
LonCorner = [XCoord[0,0], XCoord[-1,0], XCoord[-1, -1], XCoord[0, -1], XCoord[0, 0] ]
LatCorner = [YCoord[0,0], YCoord[-1,0], YCoord[-1, -1], YCoord[0, -1], YCoord[0, 0] ]


faultProjFile = open( "faultProjFile.txt", "w" )
for i in range( len( LonCorner ) ):
	faultProjFile.write( "%f %f\n" % ( LonCorner[i], LatCorner[i] )  )

faultProjFile.close( )

fig = plt.figure( 13)
plt.pcolormesh( XCoord, YCoord, ZCoord, cmap = "seismic" )
plt.colorbar( )
plt.axis( "equal" )
