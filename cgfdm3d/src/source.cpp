...
__GLOBAL__
void load_smooth_source( SOURCE S, WAVE h_W, int _nx_, int _ny_, int _nz_, float * Jac, int it, int iRK, float DT, float DH, int nGauss )
{
#ifdef GPU_CUDA
	...
	CALCULATE3D( i, j, k, S.X - nGauss, S.X + nGauss + 1, S.Y - nGauss, S.Y + nGauss + 1, S.Z - nGauss, S.Z + nGauss + 1 )   
		...
		if ( iRK == 0 )
			s = 0.0f;
		s = sourceFunction( 1.5, it, iRK, DT );
		float ra = nGauss * 0.5;
...
void loadPointSource( SOURCE S, WAVE h_W, int _nx_, int _ny_, int _nz_, float * Jac, int it, int irk, float DT, float DH )
{
		 ...
		 S.Z >= HALO && S.Z < _nz )
	{
		load_smooth_source
		<<< blocks, threads >>>
		( S, h_W, _nx_, _ny_, _nz_, Jac, it, irk, DT, DH, nGauss );
	}
