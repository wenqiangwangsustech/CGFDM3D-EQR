...
def plotGIF( plt, it, fig, XCoord, YCoord, ZCoord, Rate ):
	...
	plt.clf()
	ax = Axes3D( fig )
	handle = ax.scatter( XCoord, YCoord, ZCoord, s = 10, c = Rate[:,it], vmin = 0.0, vmax = 1.2, cmap  = "jet" )
	#ax.scatter( XTemp1, YTemp1, 0.0, s = 10, c = 'k' )
	#ax.scatter( XTemp2, YTemp2, 0.0, s = 10, c = 'k' )
	ax.set_box_aspect((8,1,1)) # 设定坐标轴的长宽高比例
	ax.grid(False) # 不要网格
...
fileTotal = open( "gnss_smv_vwest_4.8_veast_2.8.0010.inv.total", "r" )
file = open( "gnss_smv_vwest_4.8_veast_2.8.0010.inv", "r" )
linesTotal = [ ]
lines = [ ]
numLine = 0
while True:
...
sourceFileName = "./TurkeyMw7_6.bin"
writeData( sourceFileName, LonCoord, LatCoord, ZCoord, Area, Strike, Dip, Rake, Rate )


faultTrace = open( "faultM7.6.trace", "w" )
for j in range( NY1 ):
#for i in range( NX1 ):
	i = 0
	index = i + j * NX1
	faultTrace.write( "%f %f\n" % ( LonCoord[index], LatCoord[index] ) )
...
for j in range( NY2 ):
	#for i in range( NX2 ):
	i = 0
	index = i + j * NX2 + NX1 * NY1
	faultTrace.write( "%f %f\n" % ( LonCoord[index], LatCoord[index] ) )

faultTrace.close( )


np.save( "M7.6_faultLon.npy", LonCoord )
np.save( "M7.6_faultLat.npy", LatCoord )
np.save( "M7.6_faultZCoord.npy", ZCoord )
np.save( "M7.6_Slip_total.npy", Slip_total )
